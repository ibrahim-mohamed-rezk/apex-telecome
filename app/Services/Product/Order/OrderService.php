...
class OrderService
{
    public function calculate($requestData)
    {
        ...
        //     $total =  $offer['total'];
        // }
        $total_delivery = settings('shipping_cost') ;
        $tax_value = settings('tax_cost') ;
        return
        [
    ...
    private function checkCobon($cobon,$total,$cart)
    {
        ...
        $cobon = Cobon::where('code',$cobon)->where('start_date','<=',date('Y-m-d'))->where('end_date','>=',date('Y-m-d'))->orderBy('id','DESC')->first();
        if($cobon && $cobon->type == "product")
        {
            ...
            foreach($cart['carts'] as $cartItem)
            {
                ...
                if (in_array($productId, $ids)) {
                    ...
                    {
                        $discount = round( $cobon->discount ,2);
                        $msg = __('web.cobon_discount').' ' . $discount . __('web.currency');
                    }
                    else
                    {
                        $discount =  round( $total * ($cobon->discount/100) ,2);
                        $msg = __('web.cobon_discount').' ' . $discount . __('web.currency');
                    }
                    $total -= $discount;
                    $res['msg'] = $msg;
